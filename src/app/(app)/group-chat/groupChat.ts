...
export interface ProDirectoryEntryDTO {
  id: string;
  name: string;
  phone: string | null;
  email: string | null;
...
// Staff roles that may be added to channels / DMs (everyone but clients).
...
// Can moderate / manage channels + settings: office roles only. Intentionally
// narrower than role-routing's isAdminRole (which also grants FIELD_LEAD the
// admin app) — a field lead can post & DM but cannot moderate the crew.
 ...
 * managers see everything (moderation); everyone else must have a member row.
 */
...
/** Current team-chat settings. Any staff member may read (gates the Pro UI). */
...
const DEFAULT_CHANNEL_NAME = "All Pros";
 ...
 * channels for managers + their members, DMs for their two members (managers
 * see them too, for moderation). Default first, then groups, then DMs.
 */
export async function listGroupChannels(): Promise<Result<GroupChannelDTO[]>> {
  ...
  const manager = canManage(a.role);
  ...
  // the other member's name (or both names for a moderating manager).
    ...
    data: sorted.map((c) => {
      ...
      if (c.isDirect) {
        ...
        if (others.length !== c.members.length) {
          ...
          name =
            others.map((m) => nameById.get(m.userId) ?? "Unknown").join(", ") ||
            c.name;
        } else {
          // Moderating manager who is not a member — show both participants.
          name =
            c.members
...
/** Members of a custom channel (with names). Managers only — powers the editor. */
...
/** Pros an admin can add to a custom channel. Managers only. */
export async function listProsForChannel(): Promise<Result<ChannelMemberDTO[]>> {
  const a = await requireUser();
  if ("error" in a) return { success: false, error: a.error };
  if (!canManage(a.role)) return { success: false, error: "Not authorized" };
  ...
  return {
    success: true,
    data: pros.map((c) => ({ userId: c.id, name: c.name })),
  };
}

/**
 * Directory of other Pros for the "start a chat" flow. Contact info is
 * included only when the manager setting allows it; when DMs are disabled the
 * directory is empty (server-side gate, not just UI).
 */
export async function listProDirectory(): Promise<
  Result<{
    dmEnabled: boolean;
    showContactInfo: boolean;
    pros: ProDirectoryEntryDTO[];
  }>
> {
  const a = await requireUser();
  if ("error" in a) return { success: false, error: a.error };
  ...
  if (!settings.dmEnabled && !canManage(a.role)) {
    return {
      success: true,
      data: {
        dmEnabled: false,
 ...
 * Managers only. Member ids are validated against staff users — unknown ids
...
/** Add a member to a custom (non-default, non-direct) channel. Managers only. */
export async function addChannelMember(
  ...
  if (channel.isDefault) {
    return {
      success: false,
      error: "Everyone already belongs to the default channel",
    };
  }
  if (channel.isDirect) {
...
/** Remove a member from a custom channel. Managers only. */
...
/** Soft-delete (moderate) a message. Managers only. */
