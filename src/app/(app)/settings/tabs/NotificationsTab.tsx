...
export default function NotificationsTab({ settings }: NotificationsTabProps) {
  ...
  return (
    <div className="space-y-4">
      {/* Header */}
      <div className="rounded-2xl border border-[#1c1917]/10 bg-white p-5 shadow-sm">
        <div className="flex items-start gap-3">
          <div className="w-10 h-10 rounded-xl bg-[#e85d04]/8 flex items-center justify-center flex-shrink-0">
          ...
          <button
            ...
            disabled={reseeding}
            className="inline-flex items-center gap-2 text-xs font-semibold text-[#1c1917] bg-[#e85d04]/8 hover:bg-[#e85d04]/14 px-3 py-2 rounded-lg disabled:opacity-50">
            <RefreshCw className={`w-3.5 h-3.5 ${reseeding ? "animate-spin" : ""}`} />
            {reseeding ? "Refreshing…" : "Refresh catalog"}
          </button>
        </div>
        {reseedMsg && (
          <p className="text-xs text-[#1c1917]/70 mt-3">{reseedMsg}</p>
        )}
      </div>

      {/* Recipient tabs */}
      <div className="flex gap-2 border-b border-[#1c1917]/10">
        {recipients.map((r) => {
            ...
            <button
              ...
              className={`px-4 py-2.5 text-sm font-semibold border-b-2 transition-colors ${
                active
                  ...
                  : "border-transparent text-[#1c1917]/55 hover:text-[#1c1917]"
              }`}>
              {RECIPIENT_LABELS[r]}
              <span
                className={`ml-1.5 text-[10px] font-bold px-1.5 py-0.5 rounded-full ${
                  active ? "bg-[#e85d04] text-white" : "bg-[#e85d04]/8 text-[#1c1917]/70"
                }`}>
                {count}
              </span>
            </button>
      ...
      {!categoriesForActive || categoriesForActive.size === 0 ? (
        <div className="text-sm text-[#1c1917]/60 p-6 text-center">
        ...
        Array.from(categoriesForActive.entries())
          ...
          .map(([category, notifs]) => {
            ...
            return (
              <div
                key={category}
                className="rounded-2xl border border-[#1c1917]/10 bg-white overflow-hidden shadow-sm">
                <div className="px-5 py-3 bg-[#e85d04]/4 border-b border-[#1c1917]/10 flex items-center justify-between gap-3">
                    ...
                    {channelsPresent.map((channel) => {
                      ...
                      return (
                        <button
                          ...
                          className="text-[10px] font-semibold text-[#1c1917]/70 hover:text-[#1c1917] px-2 py-1 rounded-md hover:bg-[#e85d04]/8">
                ...
                <ul className="divide-y divide-[#e85d04]/8">
                  {notifsArr.map((n) => (
                    <li key={n.key} className="px-5 py-3.5 flex items-start gap-4">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-[#1c1917]/60 mt-0.5">{n.trigger}</p>
                      </div>
                        ...
                        {(["EMAIL", "SMS", "APP_PUSH"] as Channel[]).map((channel) => {
                          ...
                          if (!row) {
                            return (
                              <div
                                key={channel}
                                className="w-[88px] text-center text-[10px] text-[#1c1917]/30 py-1.5"
                            ...
                            <button
                              ...
                              className={`inline-flex items-center gap-1.5 text-[11px] font-semibold px-2.5 py-1.5 rounded-lg border transition-colors min-w-[88px] justify-center ${
                                row.enabled
                                  ...
                                  : "bg-white border-[#1c1917]/15 text-[#1c1917]/45 hover:border-[#1c1917]/30"
                              }`}
