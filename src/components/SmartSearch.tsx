...
export default function SmartSearch<T extends SmartSearchItem>({
  ...
  const defaultRenderItem = (item: T, isSelected: boolean) => {
    ...
    return (
      <>
        <span className={`${itemTextSize} font-[350] text-[#1c1917]`}>
          {item.name}
        </span>
        {isSelected && (
  ...
  const dropdownContent =
    isOpen && searchQuery.trim() && dropdownPosition ? (
      <div
        ref={dropdownRef}
        className="fixed max-h-60 overflow-y-auto rounded-2xl bg-white border border-[#1c1917]/10 shadow-lg z-[9999]"
        style={{
          ...
          filteredItems.map((item) => {
            ...
            return (
              <div
                ...
                } cursor-pointer transition-colors border-b border-[#1c1917]/5 last:border-b-0 ${
                  isSelected ? "bg-[#e85d04]/5" : "hover:bg-[#e85d04]/3"
                }`}
  ...
  return (
    <div className={`space-y-3 ${className}`}>
      ...
      <div ref={containerRef} className="relative">
        <SearchIcon
          className={`absolute ${sizeClasses.icon} top-1/2 -translate-y-1/2 z-10 text-[#1c1917]/50`}
        />
        <Input
          ref={inputRef}
          variant="form"
      ...
      {selectedIds.length > 0 && (
        <div className="space-y-2">
          <p className={`${sizeClasses.label} font-[350] text-[#1c1917]/70`}>
            {selectedLabel}
          </p>
          <div className="flex flex-wrap gap-2">
            {selectedIds.map((itemId) => {
              ...
              return (
                <div
                  key={itemId}
                  className={`flex items-center gap-2 ${sizeClasses.selectedBadge} rounded-2xl bg-[#e85d04]/10`}>
                  {renderSelectedItem
                  ...
                  <button
                    ...
                    disabled={disabled}
                    className="text-[#1c1917]/60 hover:text-[#1c1917] transition-colors disabled:opacity-50 cursor-pointer">
                    <X
