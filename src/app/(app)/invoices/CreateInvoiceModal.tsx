  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
      <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-xl mx-4">
        <div className="flex items-center justify-between p-6 border-b border-[#1c1917]/10">
          <h2 className="text-xl font-[350] text-[#1c1917]">New Invoice</h2>
          <button onClick={onClose} className="p-2 hover:bg-[#e85d04]/5 rounded-xl">
            <X className="w-5 h-5 text-[#1c1917]/60" />
          </button>
        </div>
        ...
        <div className="p-6 space-y-5">
          ...
          {/* Client */}
          <div>
            <label className="input-label !text-[#1c1917]/70 block mb-1.5">Client</label>
            <PremiumSelect
              value={clientId}
              onChange={(v) => {
                setClientId(v);
          ...
          {/* Due Date */}
          <div>
            <label className="input-label !text-[#1c1917]/70 block mb-1.5">Due Date</label>
            <DatePicker
              value={dueDate}
              onChange={setDueDate}
              size="md"
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="input-label !text-[#1c1917]/70">Line Items</label>
              <button
                onClick={addLineItem}
                className="text-xs text-[#1c1917] hover:text-[#1c1917]/80 flex items-center gap-1">
            ...
            <div className="space-y-2">
              {lineItems.map((li, idx) => (
                <div key={idx} className="flex gap-2 items-start">
                  ...
                  <div className="w-24 text-right text-sm text-[#1c1917]/70 pt-2">
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <label className="input-label !text-[#1c1917]/70">
                Discount
              </label>
              <div className="flex bg-[#e85d04]/5 rounded-xl p-0.5">
                <button
                  type="button"
                  onClick={() => {
                    setDiscountMode("percent");
          ...
          {/* Notes */}
          <div>
            <label className="input-label !text-[#1c1917]/70 block mb-1.5">Notes</label>
            <textarea
              value={notes}
              onChange={(e) => setNotes(e.target.value)}
              placeholder="Optional notes..."
              rows={2}
              className="w-full px-3 py-2 text-sm bg-[#e85d04]/5 text-[#1c1917] rounded-2xl border-0 outline-none resize-none placeholder:text-[#1c1917]/40"
            />
          </div>

          {/* Totals */}
          <div className="bg-[#e85d04]/5 rounded-2xl p-4 space-y-2">
            ...
            <div className="border-t border-[#1c1917]/10 pt-2 flex justify-between text-base font-[400] text-[#1c1917]">
              <span>Total</span>
        ...
        <div className="flex gap-3 p-6 border-t border-[#1c1917]/10">
          <Button
            variant="default"
            size="md"
            border={false}
