...
export default function GroupChatClient({
  ...
  // Directory of other Pros — fetched lazily when the panel opens.
  const { data: directory } = useSWR(
    dmEnabled && directoryOpen ? "pro-directory" : null,
    async () => {
      const res = await listProDirectory();
      if (!res.success) throw new Error(res.error);
      return res.data;
    }
  );
      ...
      <div style={{ flex: 1, minHeight: 0, padding: "0 clamp(16px,5vw,32px) clamp(16px,4vw,32px)" }}>
        <div
          style={{
            ...
            background: "#fff",
            borderRadius: 18,
            boxShadow: "0 2px 24px rgba(28,25,23,0.08), 0 1px 4px rgba(0,0,0,0.04)",
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    fontSize: 13,
                    fontWeight: 600,
                    background: active ? "var(--accent)" : "rgba(232,93,4,0.07)",
            ...
            {dmEnabled && (
              <button
                ...
                style={{
                  ...
                  fontSize: 13,
                  fontWeight: 600,
                  background: directoryOpen ? "var(--accent)" : "transparent",
                  color: directoryOpen ? "#fff" : "var(--accent)",
                  transition: "background 0.15s",
                }}>
                <Plus size={14} />
                New chat
          ...
          {directoryOpen ? (
            /* Pro directory — pick someone to start a 1:1 chat with. */
            <div style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column" }}>
              <div className="chat-thread-head">
                ...
                <div className="thread-meta">
                  ...
                  <div className="thread-role">Message another Pro directly</div>
                </div>
                <button
              ...
              <div
                ...
                ) : directory.pros.length === 0 ? (
                  <p style={{ fontSize: 13, color: "var(--primary-50)", textAlign: "center", margin: "auto" }}>
                    No other Pros to message right now.
                  </p>
                ) : (
                  directory.pros.map((c: ProDirectoryEntryDTO) => (
                    <div
                      key={c.id}
                      style={{
                        display: "flex",
