  ...
  return (
    <div style={{ marginTop: 8 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: showAdd ? 0 : 8 }}>
        {addresses.map((addr) => (
          <div key={addr.id} style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "10px 12px", borderRadius: 12, background: addr.isDefault ? "rgba(196,76,3,0.06)" : "rgba(196,76,3,0.03)", border: addr.isDefault ? "1px solid rgba(196,76,3,0.15)" : "1px solid rgba(196,76,3,0.08)" }}>
            ...
            <div style={{ display: "flex", gap: 4, flexShrink: 0 }}>
              <button onClick={() => openEdit(addr)} style={{ padding: "4px 8px", borderRadius: 8, background: "transparent", border: "1px solid rgba(196,76,3,0.15)", cursor: "pointer", color: "var(--primary-60)" }}>
                <Pencil size={12} />
              </button>
      ...
      {showAdd && (
        <div style={{ padding: 16, borderRadius: 14, background: "rgba(196,76,3,0.04)", border: "1px solid rgba(196,76,3,0.1)", marginBottom: 10 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 10 }}>
            <div>
              <label style={{ fontSize: 11, fontWeight: 600, color: "var(--primary-60)", display: "block", marginBottom: 4 }}>Label</label>
              <input value={form.label} onChange={(e) => setForm((f) => ({ ...f, label: e.target.value }))} placeholder="e.g. Home, Office" style={{ width: "100%", padding: "8px 12px", borderRadius: 8, border: "1px solid rgba(196,76,3,0.2)", fontSize: 13, outline: "none", boxSizing: "border-box" }} />
            </div>
            <div>
              <label style={{ fontSize: 11, fontWeight: 600, color: "var(--primary-60)", display: "block", marginBottom: 4 }}>Apt / Unit</label>
              <input value={form.aptNumber} onChange={(e) => setForm((f) => ({ ...f, aptNumber: e.target.value }))} placeholder="optional" style={{ width: "100%", padding: "8px 12px", borderRadius: 8, border: "1px solid rgba(196,76,3,0.2)", fontSize: 13, outline: "none", boxSizing: "border-box" }} />
            </div>
          </div>
          <div style={{ marginBottom: 10 }}>
            ...
            <input value={form.address} onChange={(e) => setForm((f) => ({ ...f, address: e.target.value }))} placeholder="123 Main Street" style={{ width: "100%", padding: "8px 12px", borderRadius: 8, border: "1px solid rgba(196,76,3,0.2)", fontSize: 13, outline: "none", boxSizing: "border-box" }} />
          </div>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <button onClick={cancel} disabled={saving} style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 14px", borderRadius: 8, background: "transparent", color: "var(--primary-60)", border: "1px solid rgba(196,76,3,0.15)", cursor: "pointer", fontSize: 13 }}>
              <X size={13} />Cancel
            </button>
          </div>
        </div>
      ...
      {!showAdd && (
        <button onClick={openAdd} style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 14px", borderRadius: 8, background: "transparent", color: "var(--primary)", border: "1px dashed rgba(196,76,3,0.3)", cursor: "pointer", fontSize: 13, fontWeight: 500 }}>
