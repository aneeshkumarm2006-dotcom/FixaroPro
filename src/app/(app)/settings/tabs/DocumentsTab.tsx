...
export default function DocumentsTab({ documents, users }: DocumentsTabProps) {
  ...
  return (
    <SectionCard
      ...
      {documents.length === 0 ? (
        <p className="text-sm text-[#1c1917]/60">No documents yet.</p>
      ) : (
              ...
              <div
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span className="text-xs bg-[#e85d04]/10 text-[#1c1917] px-2 py-0.5 rounded-full">
                      v{doc.version}
                    </span>
                    {doc.fileUrl && (
                      <span className="text-xs bg-[#77C8CC]/20 text-[#1c1917] px-2 py-0.5 rounded-full">
                        File
                      </span>
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-4 mt-2 text-xs text-[#1c1917]/60 flex-wrap">
                    {doc.dueDate && (
      ...
      <Modal
        ...
        <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-1">
          ...
          {draft.assignMode === "ROLES" && (
            <div className="flex flex-wrap gap-2">
              {ROLES.map((role) => (
                <label
                  key={role}
                  className="flex items-center gap-2 text-sm text-[#1c1917] select-none">
                  <input
                    type="checkbox"
          ...
          {draft.assignMode === "USERS" && (
            <div className="max-h-48 overflow-y-auto space-y-1 p-2 rounded-xl bg-[#e85d04]/5">
              {users.length === 0 ? (
                <p className="text-xs text-[#1c1917]/60">No users found.</p>
              ) : (
                users.map((u) => (
                  <label
                    key={u.id}
                    className="flex items-center gap-2 text-sm text-[#1c1917] select-none px-2 py-1 rounded hover:bg-white/40 cursor-pointer">
                    <input
                      type="checkbox"
                    ...
                    <span className="text-xs text-[#1c1917]/50">{u.role}</span>
                  </label>
                ))
              )}
            </div>
