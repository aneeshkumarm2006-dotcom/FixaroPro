...
function appUrl(path: string) {
  const base = process.env.APP_URL ?? "https://fixaropro.com";
...
function buildHtml(opts: {
  ...
  const safeBody = opts.body
    ...
    .slice(0, 280);
  return `
  <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;background:#f7f5f0;padding:32px 16px;color:#c44c03">
    <div style="max-width:520px;margin:0 auto;background:#fff;border-radius:16px;padding:28px 24px;box-shadow:0 2px 14px rgba(0,0,0,0.05)">
      <p style="font-size:13px;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;color:#e85d04;opacity:0.7;margin:0 0 8px">New message</p>
      <h1 style="margin:0 0 16px;font-size:22px;color:#c44c03;font-weight:700">${opts.senderName} messaged you on Fixaro</h1>
      <p style="margin:0 0 8px;font-size:14px;color:#5a6e72">Hi ${opts.recipientName.split(" ")[0] ?? ""},</p>
      <blockquote style="margin:14px 0 18px;padding:14px 16px;background:#f4f8f9;border-left:3px solid #e85d04;border-radius:6px;color:#c44c03;font-size:14.5px;line-height:1.5;white-space:pre-wrap">${safeBody}</blockquote>
      <a href="${opts.ctaHref}" style="display:inline-block;padding:11px 22px;background:#e85d04;color:#fff;border-radius:10px;text-decoration:none;font-size:14px;font-weight:600">${opts.ctaLabel}</a>
...
export async function notifyChatEmail(opts: {
  ...
  if (opts.senderRole === "EMPLOYEE") {
    ...
    const subject = `Technician (${opts.senderName}) messaged you on Fixaro`;
    const ctaHref = appUrl(`/chat?employeeId=${convo.employee.id}`);
      ...
      try {
        await resend.emails.send({
          ...
          html: buildHtml({
            ...
            senderName: `Technician ${opts.senderName}`,
            body: opts.body,
            ctaLabel: "Open chat",
            ctaHref,
          }),
    ...
    try {
      await resend.emails.send({
        ...
        subject: `${opts.senderName} messaged you on Fixaro`,
        html: buildHtml({
