...
export default function ProviderInvoiceView() {
  ...
  return (
    <Card variant="cleano_light" className="p-6">
      <div className="flex items-start justify-between gap-4 mb-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-[400] text-[#1c1917]">
              Provider Invoice
            </h2>
          </div>
      ...
      {invoice && (
        <div>
          <div className="rounded-2xl bg-white border border-[#1c1917]/10 p-5">
            <div className="flex items-center justify-between mb-4">
              <div>
                <p className="text-xs uppercase tracking-wider text-[#1c1917]/50">
                  Invoice
                </p>
                <p className="text-base font-[500] text-[#1c1917]">
            ...
            {invoice.lines.length === 0 ? (
              <p className="text-sm text-[#1c1917]/60 py-6 text-center">
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full text-sm">
                  <thead>
                    <tr className="text-[10px] uppercase tracking-wider text-[#1c1917]/50">
                  ...
                  <tbody className="divide-y divide-[#e85d04]/10">
                    {invoice.lines.map((l) => (
                      <tr key={l.payoutId}>
                        <td className="py-2 pr-3 text-[#1c1917]">
                          {formatDate(l.periodStart)} –{" "}
                          {formatDate(l.periodEnd)}
                        </td>
                        <td className="py-2 px-3 text-right text-[#1c1917]/80">
                          {l.jobCount}
                        </td>
                        <td className="py-2 px-3 text-right text-[#1c1917]/80">
                          {l.totalHours.toFixed(1)}
                        </td>
                        <td className="py-2 pl-3 text-right text-[#1c1917] font-[500]">
                  ...
                  <tfoot>
                    <tr className="border-t-2 border-[#1c1917]/30">
                      <td className="py-3 pr-3 text-[#1c1917] font-[500]">
                        Total
                      </td>
                      <td className="py-3 px-3 text-right text-[#1c1917]/80">
                        {invoice.totalJobs}
                      </td>
                      <td className="py-3 px-3 text-right text-[#1c1917]/80">
                        {invoice.totalHours.toFixed(1)}
                      </td>
                      <td className="py-3 pl-3 text-right text-[#1c1917] text-base font-[500]">
