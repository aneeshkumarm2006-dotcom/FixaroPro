  ...
  return (
    <div className={isAdmin ? "max-w-[80rem] mx-auto space-y-6" : undefined}>
      <div className={isAdmin ? "flex items-start justify-between gap-4 flex-wrap" : "cl-page-head"}>
        <div>
          <h1 className={isAdmin ? "text-3xl !font-light tracking-tight text-[#1c1917]" : "cl-page-title"}>
            Training
          </h1>
          <p className={isAdmin ? "text-sm text-[#1c1917]/70 !font-light mt-1" : "cl-page-sub"}>
            {isAdmin
      ...
      {msg && (
        <div
          className={`px-4 py-3 rounded-xl text-sm font-[350] ${
            msg.type === "success"
              ? "bg-[#e85d04]/10 text-[#1c1917] border border-[#1c1917]/15"
              ...
              <div>
                ...
                <div className="space-y-3">
                  {draft.quizzes.map((q, qIdx) => (
                    <div
                      key={qIdx}
                      className="p-3 rounded-xl bg-[#e85d04]/5 space-y-3">
                            ...
                            <button
                              ...
                              className={`p-1.5 rounded-lg border ${
                                opt.isCorrect
                                  ...
                                  : "bg-white border-[#1c1917]/20 text-[#1c1917]/40"
                              }`}
                      ...
                      <tbody className="divide-y divide-gray-50">
                        {statsModule.progress.map((p) => (
                          <tr key={p.id} className="hover:bg-gray-50/50">
                            <td className="px-4 py-2 text-[#1c1917]">
                              {p.employee.name}
                            </td>
                            <td className="px-4 py-2 text-[#1c1917]/80">
...
function FormField({
  ...
  return (
    <div>
      ...
      {children}
      {hint && <p className="text-xs text-[#1c1917]/60 mt-1">{hint}</p>}
    </div>
  );
}
    ...
    <div className="space-y-3">
      ...
      {modules.length === 0 ? (
        <Card variant="ghost" className="p-8">
          <p className="text-sm text-[#1c1917]/60 text-center">{emptyText}</p>
        </Card>
      ) : (
        <div className="space-y-2">
          {modules.map((m) => {
...
function AdminModuleRow({
  ...
  return (
    <div className="flex items-start gap-3 p-4 border border-[#1c1917]/10 rounded-2xl bg-white hover:bg-[#e85d04]/3 transition-colors">
      <div className="flex flex-col gap-1 pt-1">
        <button
          type="button"
          onClick={() => onMove(idx, -1)}
          disabled={idx <= 0}
          className="p-1 rounded text-[#1c1917]/60 hover:bg-[#e85d04]/10 disabled:opacity-20 disabled:cursor-not-allowed"
          aria-label="Move up">
      ...
      <div
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <h3 className="text-sm font-[400] text-[#1c1917]">{m.title}</h3>
          {!m.isActive && (
        ...
        {m.description && (
          <p className="text-xs text-[#1c1917]/60 mt-1 line-clamp-2">
            {m.description}
          </p>
        )}
        <div className="flex items-center gap-4 mt-2 text-xs text-[#1c1917]/60">
          ...
          <button
            ...
            className="text-[#1c1917] hover:underline inline-flex items-center gap-1">
