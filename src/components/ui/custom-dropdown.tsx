...
export default function CustomDropdown({
  ...
  const dropdownContent = isOpen && (
    <div
      ref={dropdownRef}
      className="fixed min-w-40 bg-white border border-[#1c1917]/10 rounded-2xl !overflow-hidden shadow-lg z-[9999]"
      style={{
      ...
      <div className=" overflow-y-auto " style={{ maxHeight }}>
        {options.map((option, index) => (
          <button
            ...
            className="w-full px-4 py-3 text-sm text-[#1c1917] hover:bg-[#e85d04]/5 flex items-center gap-2 transition-colors">
            {option.icon}
            {option.label}
          </button>
        ))}
